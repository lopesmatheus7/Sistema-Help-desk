        ...
        <div class="card h-100">
            ...
            <div class="card-body p-2">
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <button id="btnGerarBoletos" class="btn bg-gradient-info w-md-30 mt-4 mb-5 btn-xs btn-lg btn-block">Gerar Boletos do aluno</button>

<script>
document.getElementById("btnGerarBoletos").addEventListener("click", function() {
    // Aqui você pode adicionar lógica para interagir com o banco de dados SQL ou a API do Asaas
    // Exemplo de redirecionamento para a página gerarcarnealuno.php
    window.location.href = "../sql/asaas/gerarcarnealuno.php";
});
</script>
        ...
        <div class="card h-100">
            ...
            <div class="card-body p-2">
                <h6 class="text-uppercase text-body text-xs font-weight-bolder"></h6>
                <h2>Mensalidades online</h2>
                <div class="d-flex justify-content-between">
                <?php
                            include ('../sql/asaas/listarCobrancaAssinatura.php')
                            ?>
                </div></div></div></div>
  <div class="tab-content">
...
// Realizar a consulta SQL para obter o nome da turma associado ao usuario_id
$queryturma = "SELECT turmas.nome AS nome_turma
          FROM usuarios
          JOIN turmas ON usuarios.id_turma = turmas.id
          WHERE usuarios.id = $usuario_id";
$resultturma = $conn->query($queryturma);

// Verificar se a consulta foi bem-sucedida
if ($resultturma) {
    // Extrair os dados da consulta
    $row = $resultturma->fetch_assoc();

    // Verificar se há resultados
    if ($row) {
        $nomeTurma = $row['nome_turma'];

        // Agora você pode usar $nomeTurma onde desejar
    } else {
    ...
    $nomeTurma = "Erro na consulta: " . $conn->error;
}
...
<div class="col-md-6">
    <form method="post" action="../sql/proc_aluno_turmas.php">
        ...
        <button type="submit" class="btn btn-info">Matricular na Turma</button>
    </form>
</div>
<div class="col-md-6">
...
<div class="col-md-6">
    <form method="post" action="../sql/proc_aluno_plano.php">
        <input type="hidden" name="usuario_id" value="<?php echo $usuario_id; ?>">
        <div class="form-group">
            <label for="tipo">Plano de automatização</label>
            <h5 class="text"><?php  ?>DESATIVADO</h5>
        </div>
        <button type="submit" class="btn btn-primary">Super Plano de cobrança automática online</button>
    </form>
</div>  
<div class="col-md-6">
