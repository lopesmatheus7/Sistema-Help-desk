...
if (!isset($_SESSION['tipo']) || $_SESSION['tipo'] != 2) {
    ...
    if (isset($_COOKIE['id']) && $_COOKIE['tipo'] == 2) {
        ...
        <div class="card h-100">
            ...
            <div class="card-body p-2">
              ...
              <h3>Mensalidades cobrança online</h3>
                <?php include ('../sql/asaas/listarCobrancaAssinaturaUsuario.php'); ?>
                <?php include ('../sql/lista_mensalidades_aluno_vizualizar_usuario.php'); ?>
                <div class="modal fade" id="cobrançaModal" tabindex="-1" role="dialog" aria-labelledby="cobrançaModalLabel" aria-hidden="true">
              <?php 
                include '../modals/nova_cobranca.php';
              ?>            
              </ul>
              </div>
            </div>
...
<div class="col-md-6">
    <form method="post" action="../sql/proc_aluno_turmas.php">
        ...
        <div class="form-group">
            <label for="tipo">Turma e horários</label>
            
                <h5><?php
$query = "SELECT DISTINCT t.nome AS nome_turma
          FROM matriculas_horarios mh
          JOIN turmas t ON mh.id_turma = t.id
          WHERE t.id_conta = $id_conta AND mh.usuario_id = $id";
...
if ($result) {
    // Verifique se há pelo menos uma turma encontrada
    if (mysqli_num_rows($result) > 0) {
        // Inicie a lista HTML
        echo "<ul>";

        // Loop através dos resultados e imprima cada nome de turma
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<li>{$row['nome_turma']}</li>";
        }
...
<div class="col-md-6">
  <form method="post" action="../sql/proc_aluno_status.php">
    ...
    <ul class="list-group">
     <form method="POST" action="../sql/proc_edit_usuario_aluno.php" enctype="multipart/form-data">
