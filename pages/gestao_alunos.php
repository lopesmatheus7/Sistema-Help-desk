<?php 
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    ...
    <?php include ('../sql/cards_alunos.php');
      include ('../sql/lista_de_alunos.php');  ?>
    
        <div class="modal fade" id="CadastroModal" tabindex="-1" role="dialog" aria-labelledby="CadastroModalLabel" aria-hidden="true">
              <?php 
                include '../modals/cadastro_alunos.php';
              ?>
            </div>
    </div>
            </div>
...
<div class="card-body px-0 pb-2">
    ...
    echo "<td class='text-xs font-weight-bold mb-0'><a class='text-success' href='https://api.whatsapp.com/send?phone=55{$user_data['contato']}' target='_blank'><img src='../assets/img/small-logos/whatsapp.png' alt='Ícone do WhatsApp' style='width: 50px;'>{$user_data['contato']}</a></td>";
echo "<td class='text-xs font-weight-bold mb-0'>{$user_data['responsavel']}</td>"; // Adicionando a coluna responsavel
echo "<td><a class='btn bg-gradient-warning' href='profile.php?id={$user_data['id']}'>Painel do aluno</a></td>"; 
    echo "<td><a class='btn btn bg-gradient-info btn-sm' href='../sql/frequencia/proc_frequencia_manual.php?id={$user_data['id']}' onclick='return confirm(\"Tem certeza de que deseja marcar presença?\")'>Presença</a></td>";
                ...
                <div class="container">
                        <ul class="pagination">
                            ...
                            if ($paginaAtual > 1) {
                                ...
                                echo '<li class="page-item"><a class="page-link" href="gestao_alunos.php?page=' . $paginaAnterior . '">A</a></li>';
                            }
                            ...
                            for ($i = 1; $i <= $totalPaginas; $i++) {
                                ...
                                echo '"><a class="page-link" href="gestao_alunos.php?page=' . $i . '">' . $i . '</a></li>';
                            }

                            // Botão "Próxima"
                            if ($paginaAtual < $totalPaginas) {
                                $paginaSeguinte = $paginaAtual + 1;
                                echo '<li class="page-item"><a class="page-link" href="gestao_alunos.php?page=' . $paginaSeguinte . '">P</a></li>';
                            }
                            ?>
                        </ul>
                    </div>
    ...
    <div class="col-12 col-md-4 mb-2">
        <button class="btn bg-gradient-success w-100 btn-sm btn-xs btn-block" onclick="window.location.href='../pages/alunos_ativos.php'">Visualizar todos alunos ativos</button>
    </div>
    <div class="col-12 col-md-4 mb-2">
        <button class="btn bg-gradient-danger w-100 btn-sm btn-xs btn-block" onclick="window.location.href='../pages/alunos_inativos.php'">Visualizar alunos inativos</button>
    </div>
    <div class="col-12 col-md-4 mb-2">
        <button class="btn bg-gradient-info w-100 btn-sm btn-xs btn-block" onclick="window.location.href='../pages/alunos_online.php'">Inscrição realizadas online</button>
    </div>
</div>
            </div>
          </div>
